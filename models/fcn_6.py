...
class FCN_6_Model(FCN_Model):
    ...
    def add_model(self):
        ...
        with tf.variable_scope('conv3') as scope:
            ...
            conv = tf.nn.conv3d(input=pool2, filter=kernel, strides=[1,1,1,1,1], padding="SAME")
            
            bn3 = tf.layers.batch_normalization(conv + bias, axis=-1)
            relu3 = tf.nn.relu(bn3)
            
            pool3 = tf.layers.max_pooling3d(inputs=relu3, pool_size=(2,2,2), strides=(2,2,2), padding='VALID')
        ...
        with tf.variable_scope('deconv4') as scope:
            ...
            deconv = tf.nn.conv3d_transpose(drop3, filter=kernel, output_shape=[batch_size, 6, 6, 6, 20], strides=[1,2,2,2,1], padding='SAME')
        ...
        with tf.variable_scope('deconv5') as scope:
            ...
            deconv = tf.nn.conv3d_transpose(drop4, filter=kernel, output_shape=[batch_size, 12, 12, 12, 10], strides=[1,2,2,2,1], padding='SAME')
