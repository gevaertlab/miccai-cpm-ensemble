...
class FCN_Model(Model):
    ...
    def add_placeholders(self):
        self.image_placeholder = tf.placeholder(tf.float32,
                                                shape=[None, patch, patch, patch, 4])
        self.label_placeholder = tf.placeholder(tf.int32,
                                                shape=[None, patch, patch, patch])
    ...
    def add_model(self):
        ...
        with tf.variable_scope('deconv3') as scope:
            ...
            deconv3 = tf.nn.conv3d_transpose(pool2, filter = kernel, output_shape = [batch_size, patch/2, patch/2, patch/2, 10], strides = [1,2,2,2,1], padding='SAME')
            relu3 = tf.nn.relu(deconv3 + bias)
            bn3 = tf.layers.batch_normalization(relu3, axis=-1)
            
            drop3 = tf.nn.dropout(bn3, self.dropout_placeholder)
        ...
        with tf.variable_scope('deconv4') as scope:
            ...
            deconv4 = tf.nn.conv3d_transpose(drop3, filter = kernel, output_shape = [batch_size, patch, patch, patch, 2], strides = [1,2,2,2,1], padding='SAME')
            relu4 = tf.nn.relu(deconv4 + bias)
            bn4 = tf.layers.batch_normalization(relu4, axis=-1)
    ...
    def add_loss_op(self):
        ...
        reg_loss = self.config.l2 * (tf.nn.l2_loss(w3)
                                   + tf.nn.l2_loss(w4))
...
#                                    + tf.nn.l2_loss(w4)
    ...
    def add_train_op(self):
        self.train = tf.train.AdamOptimizer(learning_rate=self.config.lr).minimize(self.loss) 
    ...
    def _train(self, ex_path, sess):
        ...
        for batch, (x, y) in enumerate(fcn_data_iter(
                                       ex_path, 'fgbg', bs, nb)):
    ...
    def _segment(self, ex_path, sess):
        ...
        for batch, (i, j, k, x, y) in enumerate(fcn_data_iter(
            ...
            for idx, _ in enumerate(i):
                fy[i[idx]:i[idx]+patch,
                   j[idx]:j[idx]+patch,
                   k[idx]:k[idx]+patch] = y[idx, :, :, :]
                fpred[i[idx]:i[idx]+patch,
                      j[idx]:j[idx]+patch,
                      k[idx]:k[idx]+patch] = pred[idx, :, :, :]
                fprob[i[idx]:i[idx]+patch,
                      j[idx]:j[idx]+patch,
                      k[idx]:k[idx]+patch, :] = prob[idx, :, :, :, :]
